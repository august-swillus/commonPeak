...
if (length(args) < 3) {
  stop("Usage: Rscript step2_count_reads.R <bam_manifest.tsv> <output_dir> <narrow|broad> [paired|single]")
}
...
param <- SnowParam(workers = num_cores, type = "SOCK", progressbar = FALSE)
register(param)
cat("BiocParallel will use", num_cores, "cores (SnowParam).\n")
...
cat("Running featureCounts on", length(bam_files_all), "BAM files using", num_cores, "threads...\n")
